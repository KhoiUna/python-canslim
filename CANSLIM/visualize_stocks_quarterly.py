...
from get_roe_revenue import get_revenue_quarterly
...
def visualize_eps(stock_list):
    ...
    plt.title('Quarterly EPS Comparison')
    plt.ylabel('EPS')

    i = 0
    while i < len(stock_list):
        legends.append(stock_list[i].upper())
        eps_list = get_quarterly_eps(stock_list[i])
...
def visualize_eps_growth(stock_list):
    ...
    plt.title('Quarterly EPS Growth Comparison')
    plt.ylabel('Growth (%)')

    i = 0
    while i < len(stock_list):
        legends.append(stock_list[i].upper())
        eps_list = get_quarterly_eps(stock_list[i])  # get eps_list
...
def visualize_revenue(stock_list):
    ...
    plt.title('Quarterly REV. Comparison')
    plt.ylabel('Revenue (million $)')

    i = 0
    while i < len(stock_list):
        legends.append(stock_list[i].upper())
        revenue_list = get_revenue_quarterly(stock_list[i])
...
def visualize_revenue_growth(stock_list):
    ...
    plt.title('Quarterly REV. Growth Comparison')
    plt.ylabel('Growth (%)')

    i = 0
    while i < len(stock_list):
        legends.append(stock_list[i].upper())
        revenue_list = get_revenue_quarterly(stock_list[i])  # get revenue_list
...
def visualize_stocks_quarterly(stock_list):
    # Plot eps comparison
    plt.subplot(2, 2, 1)
